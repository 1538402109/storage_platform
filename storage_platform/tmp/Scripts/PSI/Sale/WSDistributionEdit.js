 ...
 * @author JIATU
 */
Ext.define("PSI.Sale.WSDistributionEdit", {
    ...
    initComponent: function() {
        ...
        me.editCustomerMobile = Ext.getCmp("editCustomerMobile");
        //  me.editCustomerAddress = Ext.getCmp("editDealAddress");
        me.orgName = Ext.getCmp("editOrgName");
        me.orgCode = Ext.getCmp("editOrgCode");
    },
            ...
            callback: function(options, success, response) {
                ...
                if (success) {
                    ...
                    if (data.customerMobile) {
                        Ext.getCmp("editCustomerMobile")
                            .setValue(data.customerMobile);
                    }
                    // if (data.customerAddress) {
                    //     Ext.getCmp("editCustomerAddress")
                    //         .setValue(data.customerAddress);
                    // }
                    if (data.orgName) {
                        Ext.getCmp("editOrgName")
                            .setValue(data.orgName);
                    }
                    if (data.orgCode) {
                        Ext.getCmp("editOrgCode")
                            .setValue(data.orgCode);
                    }
                    if (data.tmsUrl) {
                        Ext.getCmp("tmsUrl")
                            .setValue(data.tmsUrl);
                    }
                    ...
                    if (Ext.getCmp("editReceivingType").getValue() == 3) {
                        var priceSum = 0;
                        for (var i = 0; i < store.getCount(); i++) {
                            priceSum += store.getAt(i).get("moneyWithTax");
                        }
                        Ext.getCmp("editSummaryPrice").setValue(priceSum);
                    } else {
                        Ext.getCmp("editSummaryPrice").setValue(0);
                    }
                    me.setBillReadonly();
                } else {
    ...
    onOK: function() {
        var me = this;
        if (Ext.getCmp("editSummaryPrice").getValue() == "") {
            PSI.MsgBox.showInfo("请输入代收金额");
            return;
        }
        var store = me.getGoodsGrid().getStore();
        var jsonData = Ext.encode(Ext.pluck(store.data.items, 'data'));

        var tmsUrl = Ext.getCmp("tmsUrl").getValue();
        Ext.getBody().mask("正在提交中...");
        Ext.Ajax.request({
            url: tmsUrl + "/psiapi/Order/CreateOrder",
            //  url: "http://127.0.0.1:8091/psiapi/Order/CreateOrder",
            method: "POST",
            params: {
                customer_order_num: Ext.getCmp("editRef").getValue(),
                order_time: Ext.getCmp("editBizDT").getValue(),
                customer_Code: Ext.getCmp("editOrgCode").getValue(),
                recipients_name: Ext.getCmp("editCustomer").getValue(),
                recipients_contacts: Ext.getCmp("contact01").getValue(),
                recipients_tel: Ext.getCmp("editCustomerMobile").getValue(),
                recipients_address: Ext.getCmp("editDealAddress").getValue(),
                price: Ext.getCmp("editSummaryPrice").getValue(),
                customer_name: Ext.getCmp("editOrgName").getValue(),
                memo: Ext.getCmp("editBillMemo").getValue(),
                goods: jsonData
     ...
     */
    distributionStatusChange: function() {
        var me = this;
        Ext.getBody().mask("正在更新数据...");
        Ext.Ajax.request({
            url: PSI.Const.BASE_URL + "Home/Sale/distributionStatusChange",
            method: "POST",
            params: {
                id: Ext.getCmp("hiddenId").getValue(),
                ref: Ext.getCmp("editRef").getValue()
            },
            callback: function(options, success, response) {
                Ext.getBody().unmask();
                if (success) {
                    var data = Ext.JSON.decode(response.responseText);
                    if (data.success) {
                        PSI.MsgBox.showInfo("成功提交数据", function() {
                            me.close();
    ...
    setBillReadonly: function() {
        var me = this;
        me.__readonly = true;
        me.setTitle("<span style='font-size:160%'>申请物流配送</span>");
        ...
        if (Ext.getCmp("editReceivingType").getValue() != "3") {
            Ext.getCmp("editSummaryPrice").setReadOnly(true);
        }

        Ext.getCmp("editOrgName").setReadOnly(true);
        Ext.getCmp("editOrgCode").setReadOnly(true);
        // Ext.getCmp("editCustomerMobile").setReadOnly(true);
        // Ext.getCmp("editCustomerAddress").setReadOnly(true);
    ...
    __setCustomerExtData: function(data) {
        ...
        Ext.getCmp("contact01").setValue(data.contact01);
