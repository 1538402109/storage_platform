...
class UserController extends PSIBaseController {
	...
	public function editOrg() {
		if (IS_POST) {
			...
			$printTemplet=I("post.printTemplet");
			$soBillCheck=I("post.soBillCheck");
			//界面风格
			$interfaceStyle = I('post.soInterfaceStyle');
			if($interfaceStyle == 1){$interfaceStyle = 'classic';}else{$interfaceStyle = 'neptune';}
			$areaCode=I("post.areaCode");
		
			$params=["printTemplet"=>$printTemplet,
			"soBillCheck"=>$soBillCheck,
			"areaCode"=>$areaCode,
		];
			...
			$result = $us->editOrg($id, $name, $parentId, $orgCode, $orgType,$printUrl,$interfaceStyle,$params);
	...
	public function orgParentName() {
		if (IS_POST) {
			...
			if($data['interface_style'] === 'neptune'){
				//0 为深色风格
				$data['interface_style'] = 0;
			}else{
				//1  为普通风格
				$data['interface_style'] = 1;
			}
			$this->ajaxReturn($data);
		}
	}
	...
	public function getPrintUrl(){
		if (IS_POST) {
			...
			$us = new UserService();
			$this->ajaxReturn($us->getPrintUrl());
		}
	}
}