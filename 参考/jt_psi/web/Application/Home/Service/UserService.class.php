...
/**
	 ...
	 * @return string
	 */
	public function getLoignUserNameWithOrgName() {
		$dao = new UserDAO($this->db());
		return $dao->getLoignUserNameWithOrgName($this->getLoginUserId());
	}
	/**
	 * 获得当前登录用户的登录名
	 *
	...
	public function editOrg($id, $name, $parentId, $orgCode, $orgType,$printUrl,$interfaceStyle,$params_t) {
		if ($this->isNotOnline()) {
		...
		$params = [
				...
				"printTemplet"=>$params_t["printTemplet"],
				"soBillCheck"=>$params_t["soBillCheck"],
				"areaCode"=>$params_t["areaCode"],
		];
	...
	public function getPrintUrl(){
		$params = array(
			"orgCode" => $this->getLoginUserDataOrg(),
			"loginUserId" => $this->getLoginUserId()
		);
		
		$dao = new OrgDAO($this->db());
		
		return $dao->getPrintUrl($params);
	}
	...
	//修改打印地址
	public function editPrintUrl($id,$printUrl){
		if ($this->isNotOnline()) {
			return $this->notOnlineError();
		}
		if(!preg_match_all('([a-z]+://[^\s]+:\d+)',$printUrl)){
			return $this->bad("请输入带有http://或https://及端口号的地址,例:https://127.0.0.1:8000");
		}
		$params = [
			'print_url' => $printUrl
		];
		...
		$dao = new UserDAO($db);
		$rc = $dao->updatePrint($params);
		if ($rc) {
			$db->rollback();
			return $rc;
		}
		$log = "编辑用户: 打印地址：= {$printUrl}";
		// 记录业务日志
		$bs = new BizlogService($db);
		$bs->insertBizlog($log, $this->LOG_CATEGORY);
