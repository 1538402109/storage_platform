...
class InventoryService extends PSIBaseService {
	...
	public function inventoryList($params) {
		...
		$sql = "select g.id, g.code, g.name, g.spec, u.name as unit_name,
					 ...
					 convert(t.left_count, $fmt) as left_count,
				...
				from t_inventory v join  t_goods g on v.goods_id = g.id
				 join t_goods_unit u on g.unit_id = u.id
				 left join (select sum(left_count) left_count,goods_id from t_so_bill_detail  GROUP BY goods_id ) t on v.goods_id = t.goods_id
				where (v.warehouse_id = '%s')  and g.record_status != 0 ";
		if ($code) {
		...
		foreach ( $data as $i => $v ) {
			...
			$result[$i]["leftCount"] = $v["left_count"];
		}
	...
	public function inventoryBatchList($params) {
		if ($this->isNotOnline()) {
			return $this->emptyResult();
		}
		...
		$sql = "select g.id, g.code, g.name, g.spec, u.name as unit_name,
					convert(v.in_count, $fmt) as in_count, 
					convert(v.out_count, $fmt) as out_count, 
					convert(v.balance_count, $fmt) as balance_count, v.batch_date
				
				from t_inventory_batch v, t_goods g, t_goods_unit u
				where v.warehouse_id = '%s' and v.goods_id = '%s' 
					and v.goods_id = g.id and g.unit_id = u.id 
				order by v.id 
				limit %d, %d";
		$data = $db->query($sql, $warehouseId, $goodsId, $start, $limit);
		...
		foreach ( $data as $i => $v ) {
			...
			$result[$i]["inCount"] = $v["in_count"];
			$result[$i]["outCount"] = $v["out_count"];
			$result[$i]["batchDate"] = $v["batch_date"];
			$result[$i]["balanceCount"] = $v["balance_count"];
		}
		
		$sql = "select count(*) as cnt from t_inventory_batch" . " where warehouse_id = '%s' and goods_id = '%s'";
