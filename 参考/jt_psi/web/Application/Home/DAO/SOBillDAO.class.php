...
$sql = "";
		...
		if ($ref) {
			$sql .= " and (s1.ref like '%s') ";
			$queryParams[] = "%$ref%";
		}
		if ($fromDT) {
			$sql .= " and (s1.deal_date >= '%s')";
			$queryParams[] = $fromDT;
		}
		if ($toDT) {
			$sql .= " and (s1.deal_date <= '%s')";
		...
		if ($customerId) {
			$sql .= " and (s1.customer_id = '%s')";
			$queryParams[] = $customerId;
		}
		if ($receivingType != - 1) {
			$sql .= " and (s1.receiving_type = %d) ";
			$queryParams[] = $receivingType;
		}
		if ($goodsId) {
			$sql .= " and (s1.id in (select distinct sobill_id from t_so_bill_detail where goods_id = '%s'))";
			$queryParams[] = $goodsId;
		}
	...
	public function soBillDetailList($params) {
		...
		$sql = "select s.id, g.code, g.name,g.locality,g.guarantee_day, g.spec, convert(s.goods_count, " . $fmt . ") as goods_count,
	...
	public function updateSOBill(& $bill) {
		...
		$sql = "update t_so_bill
				set goods_money = %f, tax = %f, money_with_tax = %f,
					...
					bill_memo = '%s', input_user_id = '%s',distribution_type = %d, date_created = now()
				where id = '%s' ";
		...
		if ($id) {
			...
			if ($data) {
				...
				$sql = "select s.id, s.goods_id, g.code, g.name, g.spec, 
							convert(s.goods_count, " . $fmt . ") as goods_count, s.goods_price, s.goods_money,g.unit2_decimal,g.unit3_decimal,g.locality,g.guarantee_day,u2.name as unit2_name,u3.name as unit3_name,g.sale_price,g.sale_price2,g.sale_price3,
