...
class OrgDAO extends PSIBaseExDAO {
	...
	public function addOrg(& $params) {
		...
		$printTemplet=$params["printTemplet"];
		$soBillCheck=$params["soBillCheck"];
		$areaCode=$params["areaCode"];
		$interfaceStyle = $params['interface_style'];
		...
		if ($parentId == null) {
			...
			$sql = "insert into t_org (id, name, full_name, org_code, parent_id, data_org, org_type,print_url,print_templet,so_bill_check,area_code,interface_style)
						values ('%s', '%s', '%s', '%s', null, '%s', %d,'%s','%s',%d,'%s','%s')";
			
			$rc = $db->execute($sql, $id, $name, $fullName, $orgCode, $dataOrg, $orgType,$printUrl,$printTemplet,$soBillCheck,$areaCode,$interfaceStyle);
			if ($rc === false) {
			...
			$rc = $db->execute($sql, $id, $name, $fullName, $orgCode, $parentId, $dataOrg, $orgType,$printUrl,$printTemplet,$soBillCheck,$areaCode,$interfaceStyle);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
		}
		...
		if ($parentId == null) {
			...
			$sql = "update t_org
						set name = '%s', full_name = '%s', org_code = '%s', parent_id = null,
							org_type = %d,print_url='%s',print_templet='%s',so_bill_check=%d,area_code='%s',interface_style='%s'
						where id = '%s' ";
			$rc = $db->execute($sql, $name, $fullName, $orgCode, $orgType,$printUrl,$printTemplet,$soBillCheck,$areaCode,$interfaceStyle,$id);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
		} else {
			...
			if ($data) {
				...
				$sql = "update t_org
							set name = '%s', full_name = '%s', org_code = '%s', parent_id = '%s',
								org_type = %d,print_url='%s' ,print_templet='%s',so_bill_check=%d,area_code='%s',interface_style='%s'
							where id = '%s' ";
				$rc = $db->execute($sql, $name, $fullName, $orgCode, $parentId, $orgType,$printUrl, $printTemplet,$soBillCheck,$areaCode,$interfaceStyle,$id);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
	...
	public function allOrgs($params) {
		...
		$sql = "select id, name, org_code, full_name, data_org, org_type, print_url
				from t_org
				where parent_id is null ";
		if ($rs) {
			$sql .= " and " . $rs[0];
		...
		foreach ( $orgList1 as $i => $org1 ) {
			...
			$result[$i]["printUrl"]= $org1["print_url"];
			// 第二级
	...
	public function orgParentName($id) {
		...
		$data = $db->query("select parent_id, name, org_code, org_type, area_code, print_url,print_templet,so_bill_check,interface_style from t_org where id = '%s' ", 
				$id);
		...
		if ($data) {
			...
			$result["areaCode"] = $data[0]["area_code"];
			$result["printUrl"] = $data[0]["print_url"];
			$result["printTemplet"] = $data[0]["print_templet"];
			$result["soBillCheck"] = $data[0]["so_bill_check"];
			$result["interface_style"] = $data[0]["interface_style"];
	...
	public function getPrintUrl($params){
		$db = $this->db;
		$loginUserId=$params["loginUserId"];
		$sql = "select org_id
				from t_user
				where (id = '%s') ";
		$userData=$db->query($sql, $loginUserId);
		if($userData){
			$sql = "select print_url
			from t_org where id='%s'";
			$orgList1 = $db->query($sql, $userData[0]["org_id"]);
			if($orgList1){
