...
class ICBillDAO extends PSIBaseExDAO {
	...
	public function icBillDetailList($params) {
		...
		$sql = "select t.id, t.batch_date, g.code, g.name, g.spec, u.name as unit_name, 
	...
	public function addICBill(& $bill) {
		...
		$sql = "insert into t_ic_bill_detail(id, date_created, goods_id, goods_count, goods_money,
					show_order, icbill_id, data_org, company_id,batch_date, memo)
				values ('%s', now(), '%s', convert(%f, $fmt), %f, %d, '%s', '%s', '%s', '%s', '%s')";
		foreach ( $items as $i => $v ) {
			...
			$rc = $db->execute($sql, $this->newId(), $goodsId, $goodsCount, $goodsMoney, $i, $id, 
					$dataOrg, $companyId,$batchDate, $memo);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
		}
	...
	public function icBillInfo($params) {
		...
		if ($id) {
			...
			$items = [];
			$sql = "select t.id, g.id as goods_id,t.batch_date, g.code, g.name, g.spec, u.name as unit_name,i.balance_count ,
						convert(t.goods_count, $fmt) as goods_count, t.goods_money, t.memo
				from t_ic_bill_detail t join t_goods g  on t.goods_id = g.id  
				join t_goods_unit u on  g.unit_id = u.id
				left join  t_inventory i on t.goods_id = i.goods_id and i.warehouse_id ='%s'
				where t.icbill_id = '%s'  
				order by t.show_order ";
			
			$data = $db->query($sql,$result["warehouseId"], $id);
			foreach ( $data as $v ) {
				$items[] = [
						"id" => $v["id"],
						"goodsId" => $v["goods_id"],
	...
	public function commitICBill(& $params) {
		...
		$sql = "select goods_id, sum(convert(goods_count, $fmt)) as goods_count, sum(goods_money) goods_money
		...
		foreach ( $items as $i => $v ) {
			...
			if ($goodsCount < 0) {
				$index = $i + 1;
				return $this->bad("[{$goodsName}]盘点后库存数量不能为负数");
			}
			if ($goodsMoney < 0) {
				$index = $i + 1;
				return $this->bad("[{$goodsName}]盘点后库存金额不能为负数");
			}
			if ($goodsCount == 0) {
				if ($goodsMoney != 0) {
					$index = $i + 1;
					return $this->bad("[{$goodsName}]盘点后库存数量为0的时候，库存金额也必须为0");
				}
			}
			...
			if (! $data) {
				...
				// 删除库存生产日期/批次历史数据
				$sql = "delete from t_inventory_batch where goods_id = '%s' and warehouse_id = '%s' ";
				$rc = $db->execute($sql,$goodsId, $warehouseId);
				if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
				}
		...
		//处理生产日期批次逻辑begin
		$sql = "select goods_id, convert(goods_count, $fmt) as goods_count, goods_money,batch_date
		from t_ic_bill_detail
		where icbill_id = '%s'
		order by show_order ";
		$items = $db->query($sql, $id);
		...
		foreach ( $items as $i => $v ) {
			...
			$sql = "insert into t_inventory_batch (balance_count,out_count,batch_date,batch_num,in_count,warehouse_id,goods_id) 
			values( %f, %f,'%s','%s', %f,'%s','%s')";
			$rc = $db->execute($sql,$goodsCount,0,$batchDate,$batchDate, $goodsCount,$warehouseId,$goodsId);
			if ($rc === false) {
			return $this->sqlError(__METHOD__, __LINE__);
			}
		}
	...
	public function getFullBillDataByRef($ref) {
		...
		$sql = "select t.id, g.id as goods_id, g.code, g.name, g.spec, u.name as unit_name,
					convert(t.goods_count, $fmt) as goods_count, t.goods_money,t.batch_date,t.memo
				from t_ic_bill_detail t, t_goods g, t_goods_unit u
				where t.icbill_id = '%s' and t.goods_id = g.id and g.unit_id = u.id
				order by t.show_order ";
	...
	public function getICBillDataForLodopPrint($params) {
		...
		$sql = "select t.ref, t.bizdt, t.bill_status,t.warehouse_id,
					w.name as warehouse_name,
					u.name as biz_user_name,
					u1.name as input_user_name,
					t.date_created, t.bill_memo, t.company_id
		...
		// 明细表
		$sql = "select t.id, g.code, g.name, g.spec, u.name as unit_name,
				convert(i.balance_count, $fmt) as goods_count, t.goods_money,
				t.memo
				from t_ic_bill_detail t join t_goods g  on t.goods_id = g.id 
				join  t_goods_unit u on g.unit_id = u.id
				join t_inventory i on i.goods_id = t.goods_id and i.warehouse_id = '%s'
				where t.icbill_id = '%s'
				order by t.show_order ";
		$data = $db->query($sql, $warehouseId,$id);
		$items = array();
		foreach ( $data as $v ) {
			$items[] = [
					"goodsCode" => $v["code"],
