...
class UserDAO extends PSIBaseExDAO {
	 ...
	 * @return string
	 */
	public function getLoignUserNameWithOrgName($userId) {
		$db = $this->db;
...
/**
	...
	public function getIsCheckBill($userId) {
		...
		$sql = "select so_bill_check from t_org where id = '%s' ";
	...
	{
		$db = $this->db;
		$printId = session('loginUserId');
		$sql = "select org_id from t_user where id = '%s'";
		$res = $db->query($sql,$printId);
		$orgId = $res[0]['org_id'];
		$selSql = "select print_url from t_org where id = '%s'";
		$result = $db->query($selSql,$orgId);
		return ["print_url" => $result[0]['print_url']];
	}

	//查询用户主题风格
	public function selUi(){
		$db = $this->db;
		//查询数据苦衷是否有interface_style字段
		//$field = mysql_query('Describe 表名 字段名');//像数据库发送SQL语句
		$field = $db->query('Describe t_org interface_style');
		if(empty($field)){
			return ['interface_style'=>'classic'];
		}
		$userId = session('loginUserId');
		$sql = "select org_id from t_user where id = '%s'";
		$res = $db->query($sql,$userId);
		$orgId = $res[0]['org_id'];
		$selSql = "select interface_style from t_org where id = '%s'";
		$result = $db->query($selSql,$orgId);
		return ['interface_style'=>$result[0]['interface_style']];
	}

	//修改打印地址
	public function updatePrint($params){
		$db = $this->db;
		
		$printUrl = $params['print_url'];
		$id = session('loginUserId');
		$selSql = "select org_id from t_user where id= '%s'";
		$res = $db->query($selSql,$id);
		// dump($res);die;
		$orgId = $res[0]['org_id'];

		$sql = "update t_org set print_url = '%s' where id= '%s'";
		$rc = $db->execute($sql,$printUrl,$orgId);
		if ($rc === false) {
			return $this->sqlError(__METHOD__, __LINE__);
		}
