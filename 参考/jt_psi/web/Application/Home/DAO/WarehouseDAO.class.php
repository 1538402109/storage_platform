...
class WarehouseDAO extends PSIBaseExDAO {
	...
	public function warehouseList($params) {
		...
		$sql = "select id, code, name, inited, data_org, enabled,is_default,is_sale from t_warehouse ";
	...
	public function addWarehouse(& $params) {
		...
		$sql = "insert into t_warehouse(id, code, name, inited, py, data_org, company_id,is_default,is_sale)
					values ('%s', '%s', '%s', 0, '%s', '%s', '%s',%d,%d)";
		$rc = $db->execute($sql, $id, $code, $name, $py, $dataOrg, $companyId,$isDefault,$isSale);
		if ($rc === false) {
	...
	public function updateWarehouse(& $params) {
		...
		$isDefault = intval($params["isDefault"]);
		$isSale = intval($params["isSale"]);
		if ($this->isEmptyStringAfterTrim($code)) {
			return $this->bad("仓库编码不能为空");
		}
		...
		$sql = "update t_warehouse
				set code = '%s', name = '%s', py = '%s',
					...
					is_sale = %d
				where id = '%s' ";
		$rc = $db->execute($sql, $code, $name, $py, $enabled,$isDefault,$isSale, $id);
		if ($rc === false) {
			return $this->sqlError(__METHOD__, __LINE__);
		}
