...
class SRBillDAO extends PSIBaseExDAO {
	...
	public function selectWSBillList($params) {
		...
		$sql = "select w.id, w.ref, w.bizdt, c.name as customer_name, u.name as biz_user_name,
				 ...
				 where (w.customer_id = c.id) and (w.biz_user_id = u.id)
				 and (w.input_user_id = user.id) and (w.warehouse_id = h.id)
				 and (w.bill_status = 1000 or w.bill_status = 2000 ) ";
		$queryParamas = [];
	...
	public function getWSBillInfoForSRBill($params) {
		...
		$sql = "select c.name as customer_name, w.ref, h.id as warehouse_id,
					h.name as warehouse_name, c.id as customer_id,w.biz_user_id,u.login_name as biz_user_name
				from t_ws_bill w, t_customer c, t_warehouse h,t_user u
				where w.id = '%s' and w.customer_id = c.id and w.warehouse_id = h.id and w.biz_user_id = u.id ";
		$data = $db->query($sql, $id);
		if (! $data) {
			return $result;
		}
	...
	public function addSRBill(& $bill) {
		...
		//taoys  20200517修改 不校验出库单。支持不管理出库单直接创建退库单
		...
		$rc = $db->execute($sql, $id,$billStatus, $bizDT, $bizUserId, $customerId, $loginUserId, $ref, 
		...
		foreach ( $items as $i => $v ) {
			...
			else//这种情况算是直接创建的退库单 从库存中获取
			{
				...
				$data = $db->query($sql, $goodsId,$warehouseId);
	...
	public function commitSRBill(& $params) {
		...
		foreach ( $data as $i => $v ) {
			...
			if(!$wsbillDetailId){//没有出库详情ID说明是直接不关联出库单的退库单。不验证数量
				continue;
			}
			// 退货数量为负数
			if ($rejCount < 0) {
			...
			$sql = "select sum(convert(d.rejection_goods_count, $fmt)) as rej_count
					...
					where s.id = d.srbill_id and s.bill_status <> 0 and s.bill_status <> 500 
		...
		if ($paymentType == 0||$paymentType ==3) {
			...
			$data = $db->query($sql, $customerId, $companyId,$loginUserDataOrg);
			if ($data) {
				...
				$sql = "insert into t_payables (id, ca_id, ca_type, pay_money, balance_money,
							act_money, company_id,data_org)
						values ('%s', '%s', 'customer', %f, %f, %f, '%s','%s')";
				$rc = $db->execute($sql, $this->newId(), $customerId, $rejectionSaleMoney, 
						$rejectionSaleMoney, 0, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
			}
			...
			$sql = "insert into t_payables_detail(id, ca_id, ca_type, pay_money, balance_money,
					biz_date, date_created, ref_number, ref_type, act_money, company_id,data_org)
					values ('%s', '%s', 'customer', %f, %f,
					 '%s', now(), '%s', '销售退货入库', 0, '%s','%s')";
			$rc = $db->execute($sql, $this->newId(), $customerId, $rejectionSaleMoney, 
					$rejectionSaleMoney, $bizDT, $ref, $companyId,$loginUserDataOrg);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
			...
			$data = $db->query($sql, $bizDT, $companyId,$loginUserDataOrg);
			if (! $data) {
				// 当天首次发生现金业务
				...
				$sql = "insert into t_cash(out_money, balance_money, biz_date, company_id,data_org)
						values (%f, %f, '%s', '%s','%s')";
				$rc = $db->execute($sql, $outCash, $balanceCash, $bizDT, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
				...
				$sql = "insert into t_cash_detail(out_money, balance_money, biz_date, ref_type,
						...
						values (%f, %f, '%s', '销售退货入库', '%s', now(), '%s','%s')";
				$rc = $db->execute($sql, $outCash, $balanceCash, $bizDT, $ref, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
			} else {
				...
				$sql = "update t_cash
						...
						where biz_date = '%s' and company_id = '%s' and data_org = '%s' ";
				$rc = $db->execute($sql, $sumOutMoney, $balanceCash, $bizDT, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
			...
			$sql = "update t_cash
					...
					where biz_date > '%s' and company_id = '%s' and data_org = '%s' ";
			$rc = $db->execute($sql, $outCash, $bizDT, $companyId,$loginUserDataOrg);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
			...
			$sql = "select in_money, balance_money
					...
					where customer_id = '%s' and company_id = '%s' and data_org = '%s' ";
			$data = $db->query($sql, $customerId, $companyId,$loginUserDataOrg);
			if (! $data) {
				$totalInMoney = $inMoney;
				$totalBalanceMoney = $inMoney;
				$sql = "insert into t_pre_receiving(id, customer_id, in_money, balance_money, company_id,data_org)
						values ('%s', '%s', %f, %f, '%s','%s')";
				$rc = $db->execute($sql, $this->newId(), $customerId, $totalInMoney, 
						$totalBalanceMoney, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
			} else {
				...
				$sql = "update t_pre_receiving
						...
						where customer_id = '%s' and company_id = '%s' and data_org = '%s' ";
				$rc = $db->execute($sql, $totalInMoney, $totalBalanceMoney, $customerId, $companyId,$loginUserDataOrg);
				if ($rc === false) {
					return $this->sqlError(__METHOD__, __LINE__);
				}
			}
			...
			$sql = "insert into t_pre_receiving_detail(id, customer_id, in_money, balance_money,
						...
						company_id,data_org)
					values('%s', '%s', %f, %f, '%s', now(), '%s', '销售退货入库', '%s', '%s', '%s','%s')";
			$rc = $db->execute($sql, $this->newId(), $customerId, $inMoney, $totalBalanceMoney, 
					$bizDT, $ref, $bizUserId, $loginUserId, $companyId,$loginUserDataOrg);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
		}
