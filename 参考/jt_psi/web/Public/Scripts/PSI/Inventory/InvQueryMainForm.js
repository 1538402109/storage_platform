...
Ext.define("PSI.Inventory.InvQueryMainForm", {
    ...
    initComponent: function() {
        ...
        Ext.define("PSIInventory", {
            ...
            fields: ["id", "goodsId", "goodsCode", "goodsName", "goodsSpec",
                ...
                "balanceMoney", "afloatCount", "afloatMoney", "afloatPrice","leftCount"
            ]
        });
        ...
        Ext.define("PSIInventoryBatch", {
            extend: "Ext.data.Model",
            fields: ["id", "goodsCode", "goodsName", "goodsSpec",
                "unitName", "inCount", "inPrice", "inMoney",
                "outCount", "outPrice", "outMoney", "balanceCount",
                "balancePrice", "balanceMoney", "batchDate",
                "bizUserName", "refType", "refNumber"
            ]
        });
        Ext.apply(me, {
            ...
            items: [{
                ...
                items: [{
                    ...
                    border: 1,
                    items: [me
                        .getInventoryDetailGrid(), me.getInventoryBatchGrid()
                    ]
                }]
            }]
        });
    ...
    getInventoryBatchGrid: function() {
        var me = this;
        if (me.__inventoryBatchGrid) {
            return me.__inventoryBatchGrid;
        }

        var store = Ext.create("Ext.data.Store", {
            model: "PSIInventoryBatch",
            pageSize: 20,
            proxy: {
                type: "ajax",
                actionMethods: {
                ...
                url: PSI.Const.BASE_URL + "Home/Inventory/inventoryBatchList",
                reader: {
                    root: 'details',
                    totalProperty: 'totalCount'
                }
        ...
        me.__inventoryBatchGrid = Ext.create("Ext.grid.Panel", {
            cls: "PSI",
            title: "批次明细",
            viewConfig: {
                enableTextSelection: true
            ...
            tbar: ["->", {
                ...
                listeners: {
                    change: {
                        fn: function() {
                            store.pageSize = Ext
                                .getCmp("comboCountPerPageDetail2")
                                .getValue();
                            store.currentPage = 1;
                            Ext.getCmp("pagingtoolbarDetail2")
                                .doRefresh();
                        },
                        scope: me
                    }
    ...
    refreshInventoryGrid: function() {
        ...
        me.getInventoryBatchGrid().getStore().removeAll();
        var item = me.getWarehouseGrid().getSelectionModel().getSelection();
        if (item == null || item.length != 1) {
            return;
        }
    ...
    onInventoryGridSelect: function() {
        ...
        this.getInventoryBatchGrid().getStore().loadPage(1);
    },
