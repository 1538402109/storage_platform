 ...
 * 新增或编辑用户界面
 */
 Ext.define("PSI.Personnel.PersonnelEditForm", {
	...
	initComponent : function() {
		...
		me.editdataOrg = Ext.getCmp("editdataOrg");
	},
	...
	onWndShow : function() {
		...
		Ext.Ajax.request({
			url : me.URL("/Home/Personnel/userInfo"),
			params : {
				id : me.getEntity().id
			},
			method : "POST",
			callback : function(options, success, response) {
				if (success) {
					...
					me.editdataOrg.setValue(data.dataOrg);
				}
	...
	onOK : function() {
		...
		// console.log(me.getParentForm().getMainGrid());return;
		var el = f.getEl();
		el.mask("数据保存中...");
		f.submit({
			url : PSI.Const.BASE_URL + "Home/Personnel/editPersonnel",
			method : "POST",
