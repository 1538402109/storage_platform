...
Ext.define("PSI.Sale.SRMainForm", {
    ...
    getToolbarCmp: function() {
        ...
        return [{
            ...
            text: "审核",
            hidden: me.getPermission().verify == "0",
            scope: me,
            handler: me.onReview,
            id: "buttonReview"
        }, {
            text: "取消审核",
            hidden: me.getPermission().unVerify == "0",
            scope: me,
            handler: me.onCancelReview,
    ...
    onReview: function() {
        ...
        if (item == null || item.length != 1) {
            PSI.MsgBox.showInfo("请选择要审核的销售退货入库单");
            return;
        }
        var bill = item[0];

        if (bill.get("billStatus") == "已入库") {
            PSI.MsgBox.showInfo("当前销售退货入库单已经提交入库，不能再次审核");
            return;
        }

        var info = "请确认是否审核销售退货入库单: <span style='color:red'>" + bill.get("ref") +
            "</span>";
    ...
    onCancelReview: function() {
        ...
        var item = me.getMainGrid().getSelectionModel().getSelection();
        if (item == null || item.length != 1) {
            PSI.MsgBox.showInfo("请选择要取消审核的销售退货入库单");
            return;
        }
        var bill = item[0];

        var info = "请确认是否取消审核销售退货入库单: <span style='color:red'>" + bill.get("ref") +
            "</span>";
        PSI.MsgBox.confirm(info, function() {
            var el = Ext.getBody();
            el.mask("正在取消中...");
    ...
    onCommit: function() {
        ...
        var bill = item[0];
        if (bill.get("billStatus") == "已入库") {
            PSI.MsgBox.showInfo("当前销售退货入库单已经提交入库，不能再次提交2");
            return;
        }
    ...
    onMainGridSelect: function() {
        ...
        if (item == null || item.length != 1) {
            ...
            Ext.getCmp("buttonDelete").setDisabled(true);
            Ext.getCmp("buttonCommit").setDisabled(true);
            Ext.getCmp("buttonReview").setDisabled(true);
        ...
        Ext.getCmp("buttonReview").setDisabled(commited);
        Ext.getCmp("buttonCancelConfirm").setDisabled(commited);
        if (!commited) {
            var commited2 = bill.get("billStatus") == "待审核";
            Ext.getCmp("buttonReview").setDisabled(!commited2);
            Ext.getCmp("buttonCancelConfirm").setDisabled(commited2);
        }
        me.freshDetailGrid();
    },
