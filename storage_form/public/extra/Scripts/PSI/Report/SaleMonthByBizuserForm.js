...
Ext.define("PSI.Report.SaleMonthByBizuserForm", {
        ...
        me.__mainGrid = Ext.create("Ext.grid.Panel", {
            ...
            columns: [{
                ...
                },
                {
                    ...
                    sortable: false,
                    width: 160,
                    handler: me.onExcelDetail,
                    renderer: function(value, md, record) {
                        //    return '<a >导出详情</a>';
                        // return '<a taget='
                        // _blank '  href="Home/Report/saleMonthByBizuserPdf?limit=-1&dt="' + record.get("bizDT") + '"&uid="' + record.get("userId") + '"   onclick="onExcelDetail(this)">导出详情</a>';
                        // // return "<a  href="javascript:changeReceivable();"  onclick='onExcelDetail()'> 导出详情 < /a>";
                        return "<a href='" + PSI.Const.BASE_URL + "Home/Report/saleDetailIndex?begindate=" + encodeURIComponent(record
                                .get("bizDT")) +
                            "-01&userid=" +
                            encodeURIComponent(record.get("userId")) + "&username=" + encodeURIComponent(record
                                .get("userName")) + "&enddate=" + encodeURIComponent(record
                                .get("bizDT")) +
                            "-31' target='_blank'>销售详单</a> ";
                    }
                },
                {
                    header: "导出",
                    ...
                    renderer: function(value, md, record) {
                        ...
                        // _blank '  href="Home/Report/saleMonthByBizuserPdf?limit=-1&dt="' + record.get("bizDT") + '"&uid="' + record.get("userId") + '"   onclick="onExcelDetail(this)">导出详情</a>';
                        // // return "<a  href="javascript:changeReceivable();"  onclick='onExcelDetail()'> 导出详情 < /a>";
                        return "<a href='" + PSI.Const.BASE_URL + "Home/Report/saleMonthDetailByBizuserExcel?bizdt=" + encodeURIComponent(record
                                .get("bizDT")) +
                            "&userid=" +
                            encodeURIComponent(record.get("userId")) +
                            "' target='_blank'>日报汇总</a> ";
                    }
                }
            ],
            store: store
    ...
    onExcelDetail: function(uid) {
        ...
        var url = "Home/Report/saleMonthDetailByBizuserExcel?limit=-1&year=" + year +
            "&month=" + month +
            "&uid=" + uid;
        if (sorter) {
            url += "&sort=" + sorter;
