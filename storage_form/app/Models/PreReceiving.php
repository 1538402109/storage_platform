...
class PreReceiving extends Base
{
	use HasFactory;
	 ...
	 * @return NULL|array
	 */
	public function addPreReceiving1(& $params) {
		...
		if (! $data) {
			...
			$sql = "insert into t_pre_receiving(id, customer_id, in_money, balance_money, company_id)
					values (?, ?, ?, ?, ?)";
			$rc = DB::insert($sql, [$this->newId(), $customerId, $inMoney, $inMoney, $companyId]);
			if ($rc === false) {
			...
			$sql = "insert into t_pre_receiving_detail(id, customer_id, in_money, balance_money, date_created,
					...
					values(?, ?, ?, ?, now(), '', '收预收款', ?, ?, ?, ?)";
			$rc = DB::insert($sql, [$this->newId(), $customerId, $inMoney, $inMoney, $bizUserId, 
			...
			$sql = "update t_pre_receiving
					...
					where customer_id = ? and company_id = ? ";
			$rc = DB::update($sql, [$totalInMoney, $totalBalanceMoney, $customerId, $companyId]);
			if ($rc === false) {
				return $this->sqlError(__METHOD__, __LINE__);
			}
			...
			$sql = "insert into t_pre_receiving_detail(id, customer_id, in_money, balance_money, date_created,
						ref_number, ref_type, biz_user_id, input_user_id, biz_date, company_id)
					values(?, ?, ?, ?, now(), '', '收预收款', ?, ?, ?, ?)";
			$rc = DB::insert($sql, [$this->newId(), $customerId, $inMoney, $totalBalanceMoney, 
	...
	public function addPreReceiving($params) {
		...
		$rc = $this->addPreReceiving1($params);
		if ($rc) {
			DB::rollback();
			return $rc;
		}
	...
	public function returnPreReceiving($params) {
		...
		$rc = $this->returnPreReceiving1($params);
		if ($rc) {
			DB::rollback();
			return $rc;
		}
	 ...
	 * @return NULL|array
	 */
	public function returnPreReceiving1(& $params) {
		$companyId = $params["companyId"];
		if ($this->companyIdNotExists($companyId)) {
			return $this->badParam("companyId");
		}
		...
		$rc = DB::update($sql, [$totalOutMoney, $balanceMoney, $customerId, $companyId]);
		if ($rc === false) {
			return $this->sqlError(__METHOD__, __LINE__);
		}
		...
		$sql = "insert into t_pre_receiving_detail(id, customer_id, out_money, balance_money,
				...
				values (?, ?, ?, ?, ?, now(), '', '退预收款', ?, ?, ?)";
		$rc = DB::insert($sql, [$this->newId(), $customerId, $outMoney, $balanceMoney, $bizDT, 
	 ...
	 * @return array
	 */
	public function prereceivingList1($params) {
		...
		$sql = "select r.id, c.id as customer_id, c.code, c.name,
				...
				where r.customer_id = c.id and r.company_id = ? ";
	 ...
	 * @return array
	 */
	public function prereceivingDetailList1($params) {
		$companyId = $params["companyId"];
		if ($this->companyIdNotExists($companyId)) {
			return $this->emptyResult();
		}
		...
		$sql = "select d.id, d.ref_type, d.ref_number, d.in_money, d.out_money, d.balance_money,
				...
				where d.customer_id = ? and d.biz_user_id = u1.id and d.input_user_id = u2.id
					and (d.biz_date between ? and ?)
					and d.company_id = ?
				order by d.date_created
				limit ? , ?
				";
		$data = DB::select($sql, [$customerId, $dtFrom, $dtTo, $companyId, $start, $limit]);
		$data = json_decode(json_encode($data),true);
		$result = array();
		foreach ( $data as $i => $v ) {
			$result[$i]["id"] = $v["id"];
		...
		$data = DB::select($sql, [$customerId, $dtFrom, $dtTo, $companyId]);
		$data = json_decode(json_encode($data),true);
		$cnt = $data[0]["cnt"];
	...
	public function prereceivingList($params) {
		...
		return $this->prereceivingList1($params);
	}
	...
	public function prereceivingDetailList($params) {
		...
		return $this->prereceivingDetailList1($params);
	}
}